...
public class IPAddressInfoFragment extends Fragment
	implements OnClickListener, AsyncHTTPRequest.RequestTaskCaller {
    	...
    	if (view != null) {
        	...
        	((TextView)view.findViewById(R.id.coordinatesValue)).setText(mLatLong.length() == 0 ? "" :
        			Html.fromHtml(String.format(
        					"<a href=\"%s\">%s</a>", mapLink(mLatLong, mIPAddress), mLatLong)));
        	...
        	if (mLastUpdateTime != null) {
	        	...
	        	if (mLastUpdateTimedOut) {
	        		lastUpdateStatus.setText(
	        				String.format(getResources().getString(R.string.last_update_status_timeout), mLastUpdateElapsedTimeMs / 1000));
	        		lastUpdateStatus.setTextColor(Color.RED);
	        	} else if (mLastUpdateSucceeded) {
	        		lastUpdateStatus.setText(
	        				String.format(getResources().getString(R.string.last_update_status_success), mLastUpdateElapsedTimeMs));
	        		...
	        		lastUpdateStatus.setText(
	        				String.format(getResources().getString(R.string.last_update_status_fail), mLastUpdateElapsedTimeMs));
	        		lastUpdateStatus.setTextColor(Color.RED);
	        	}
    ...
    private boolean IPAddressRequestTaskIsPendingOrRunning() {
    	return this.mRequestTaskWeakRef != null &&
    			this.mRequestTaskWeakRef.get() != null &&
